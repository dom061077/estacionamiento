		...
		<div id="list-tarifa" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="descripcion" title="${message(code: 'tarifa.descripcion.label', default: 'Descripcion')}" />
					
						<g:sortableColumn property="precio" title="${message(code: 'tarifa.precio.label', default: 'Precio')}" />
				...
				<g:each in="${tarifaInstanceList}" status="i" var="tarifaInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${tarifaInstance.id}">${fieldValue(bean: tarifaInstance, field: "descripcion")}</g:link></td>
					
						<td>${fieldValue(bean: tarifaInstance, field: "precio")}</td>
